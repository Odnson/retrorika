...
export default function HeroSection({ isLoaded = false }: HeroSectionProps) {
  ...
  // Text moves up from coffee cup on scroll - slower progression
  const textY = useTransform(scrollYProgress, [0, 0.35], ["20vh", "0vh"]);
  ...
  // Hero slide up dan fade out saat dissolve - slower
  const heroSlideUp = useTransform(scrollYProgress, [0.7, 0.88], ["0%", "-70%"]);
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        // Clamp alpha to prevent white flashing
    ...
    // Cleanup existing context first
    if (sceneRef.current) {
    ...
    // Store refs and start render loop
    let animationId: number;
    let isRendering = false;
    
    const render = () => {
      if (sceneRef.current && isRendering) {
  ...
  // Track if dissolve is active for render loop control
  ...
  // Render loop that only runs during dissolve
  ...
  const stopRenderLoop = useCallback(() => {
    ...
    // Final render to ensure correct state
    if (sceneRef.current) {
      sceneRef.current.renderer.render(sceneRef.current.scene, sceneRef.current.camera);
    }
  }, []);
  ...
  useEffect(() => {
    const unsubscribe = scrollYProgress.on("change", (latest) => {
      if (sceneRef.current) {
        // Map scroll progress to shader dissolve - slower progression
        ...
        // Start/stop render loop based on dissolve state
  ...
  }, [scrollYProgress, startRenderLoop, stopRenderLoop]);

  return (
    <motion.div 
      ref={containerRef} 
      style={{ height: "320vh", marginBottom: "-120vh"}} 
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0">
          <img
            src="/kapucinobaru.jpg.webp"
            alt="Retrorika Cafe - Cappucino art dengan latte art berkualitas tinggi"
        ...
        <div 
          className="absolute z-21 pointer-events-none md:hidden"
          style={{
            left: "50%",
        ...
        >
          <img
            src="/kapucinobaru-potong.webp"
            alt="Cangkir kopi Retrorika"
        ...
        <div 
          className="absolute z-21 pointer-events-none hidden md:block lg:hidden"
          style={{
            left: "50%",
            transform: "translateX(-50%)",
            /* === TABLET SETTINGS === */
        ...
        <div 
          className="absolute z-21 pointer-events-none hidden lg:block"
          style={{
            left: "50%",
            transform: "translateX(-50%)",
            /* === DESKTOP SETTINGS === */
        ...
        {/* Hero text — MOBILE version - optimized with single animation */}
        <motion.div
